...
export default function MeetPage({ currentUser }) {
  ...
  useEffect(() => {
    ...
    const connectRoom = async () => {
      if (!liveKitToken) {
        setMeetingState('error');
        setMeetingError(
          'LiveKit token is required. Add ?token=... to the meeting URL or return a LiveKit token in the signed-in session.'
        );
        return;
      }
  ...
  }, [currentUser, liveKitToken, roomId, stopMeetingRecording, syncParticipantTiles]);
