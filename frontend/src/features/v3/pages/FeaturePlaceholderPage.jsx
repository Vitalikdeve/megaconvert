  ...
  return (
    <section className="mc-card rounded-3xl p-6 md:p-8">
      <div className="inline-flex items-center gap-2 rounded-full border border-blue-200/70 dark:border-blue-400/20 bg-blue-50/80 dark:bg-blue-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-blue-700 dark:text-blue-200">
        <Sparkles size={13} />
        {eyebrow || t('sharedUi.featurePlaceholder.eyebrowFallback')}
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-white/40 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-2xl p-4">
          <div className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-400">{t('sharedUi.featurePlaceholder.techStack')}</div>
          <div className="mt-3 flex flex-wrap gap-2">
            {(stack.length ? stack : [t('sharedUi.featurePlaceholder.tbd')]).map((item) => (
        ...
        <div className="rounded-2xl border border-white/40 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-2xl p-4">
          <div className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-400">{t('sharedUi.featurePlaceholder.rolloutStage')}</div>
          <div className="mt-3 space-y-2">
            {(milestones.length ? milestones : [t('sharedUi.featurePlaceholder.skeletonCreated')]).map((item) => (
