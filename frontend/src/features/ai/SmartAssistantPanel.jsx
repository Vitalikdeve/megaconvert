...
export default function SmartAssistantPanel({
  ...
  const buttonLabel = stageLabel || 'Отправить';
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 ai-panel">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold text-slate-900">Умный ассистент конвертации</h3>
          <p className="mt-1 text-sm text-slate-600">
            Загрузите файл, опишите задачу обычным языком, и ассистент сам подберет нужный маршрут конвертации.
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        >
          {file ? (
            <div className="text-left">
              <div className="text-xs uppercase tracking-widest text-slate-500">Файл загружен</div>
            ...
            <div>
              <div className="text-base font-semibold text-slate-900">Перетащите файл сюда</div>
              <div className="mt-2 text-sm text-slate-500">или выберите файл вручную</div>
              <button
                type="button"
                onClick={onBrowseClick}
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <label className="text-xs uppercase tracking-widest text-slate-500">Запрос ассистенту</label>
          <textarea
            value={prompt}
            onChange={(event) => onPromptChange?.(event.target.value)}
            rows={6}
            placeholder="Например: переделай эту пдф в ворд"
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            {intent?.from && intent?.to && (
              <div className="text-xs text-slate-600">
                Маршрут: <strong>{intent.from}</strong> → <strong>{intent.to}</strong>
              </div>
            )}
          </div>
